...
def get_fake_data():
    return [
        ...
        {"year": 2022, "total_clp": 2000},
        {"year": 2023, "total_clp": 3000}
    ]
...
def test_generate_sales_by_year_plot_raises_with_missing_columns(mock_savefig):
    incomplete_data = [
        {"year": 2022},  # falta 'total_clp'
        {"total_clp": 2000}  # falta 'year'
    ]
    try:
        generate_sales_by_year_plot(incomplete_data)
    except ValueError as e:
        assert str(e) == "Missing required columns in data: 'Year' and 'total_clp'."
    else:
